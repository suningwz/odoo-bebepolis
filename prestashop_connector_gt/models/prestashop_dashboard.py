...
class SaleShop(models.Model):
    ...
    def _presta_kanban_dashboard(self):
        ...
        pending_order_ids = order_obj.search([('shop_id','=',self[0].id),('presta_id','!=',False),('state','in',('sale','sent'))])
        complete_order_ids = order_obj.search([('shop_id','=',self[0].id),('presta_id','!=',False),('state','=','done')])
        draft_order_ids = order_obj.search([('shop_id','=',self[0].id),('presta_id','!=',False),('state','=','draft')])
        ...
        draft_invoice_ids = invoice_obj.search([('is_prestashop','=',True),('invoice_origin', 'in', origin_list),('state','=','draft')])
        cancel_invoice_ids = invoice_obj.search([('is_prestashop','=',True),('invoice_origin', 'in', origin_list),('state','=','cancel')])
        
        all_stock_ids = stock_obj.search([('is_presta','=',True), ('origin', 'in', origin_list)])
        pending_stock_ids = stock_obj.search([('is_presta','=',True),('origin', 'in', origin_list),('state','in', ('confirmed', 'waiting'))])
        complete_stock_ids = stock_obj.search([('is_presta','=',True),('origin', 'in', origin_list),('state','=','done')])
