...
from odoo.addons.prestashop_connector_gt.prestapyt.prestapyt import \
    PrestaShopWebServiceError as PrestaShopWebServiceError
...
class SaleShop(models.Model):
    ...
    def create_presta_category(self, prestashop, category):
        ...
        prod_category_obj = self.env['prestashop.category']
        category_check = prod_category_obj.search(
            [('presta_id', '=', self.get_value_data(category.get('category').get('id')))])
        try:
            if not category_check:
                vals = {'presta_id': self.get_value_data(category.get('category').get('id'))[0],
                        'name': self.get_value_data(
                            self.get_value(category.get('category').get('name').get('language'))), }

                parent_category_check = prod_category_obj.search(
                    [('presta_id', '=', self.get_value_data(category.get('category').get('id_parent')))])
                ...
                if not parent_category_check:
                    ...
                    if self.get_value_data(category.get('category').get('id_parent'))[0] != '0':
                        ...
                        valsss = prestashop.get('categories',
                                                self.get_value_data(category.get('category').get('id_parent')))
                        parent_id = self.get_categ_parent(prestashop, valsss)[0].id
                    else:
                        parent_id = False
                    if parent_id:
                        self.env.cr.execute(
                            "select categ_id from presta_categ_shop_rel where categ_id = %s and shop_id = %s" % (
                                parent_id, self.id))
                        categ_data = self.env.cr.fetchone()
                        if categ_data == None:
                            self.env.cr.execute(
                                "insert into presta_categ_shop_rel values(%s,%s)" % (parent_id, self.id))
                ...
                if category_check[0]:
                    ...
                    if categ_data == None:
                        self.env.cr.execute(
                            "insert into presta_categ_shop_rel values(%s,%s)" % (category_check[0].id, self.id))
    ...
    def create_customer(self, customer_detail, prestashop):
        ...
        try:
            ...
            vals = {
                'presta_id': presta_id,
                'name': self.get_value_data(
                    customer_detail.get('customer').get('firstname')) + ' ' + self.get_value_data(
                    customer_detail.get('customer').get(
                        'lastname')) or ' ',
            ...
            if self.get_value_data(customer_detail.get('customer').get('id_lang')):
                lang_values = prestashop.get('languages',
                                             self.get_value_data(customer_detail.get('customer').get('id_lang')))
    ...
    def create_presta_supplier(self, supplier_detail):
        ...
        try:
            ...
            supp_ids = res_partner_obj.search(
                [('presta_id', '=', self.get_value_data(supplier_detail.get('supplier').get('id'))),
                 ('supplier_rank', '=', True), ('customer_rank', '=', False), ('manufacturer', '=', False)])
            if not supp_ids:
    ...
    def create_presta_manufacturers(self, manufacturer_detail):
        ...
        try:
            ...
            manufact_ids = res_partner_obj.search(
                [('presta_id', '=', self.get_value_data(manufacturer_detail.get('manufacturer').get('id'))),
                 ('manufacturer', '=', True)])
            if not manufact_ids:
                ...
                for attrs in add_list:
                    ...
                    if data:
                        ...
                        addr_vals = {
                            # 'parent_id': data,
                            'name': self.get_value_data(
                                address.get('address').get('firstname')) + ' ' + self.get_value_data(
                                address.get('address').get('lastname')),
                            ...
                            'supplier_rank': self.get_value_data(
                                address.get('address').get('id_supplier')) == '1' and address.get('address').get(
                                'id_supplier') or False,
                            # 'manufacturer': self.get_value_data(address.get('address').get('id_manufacturer'))[0],
                            'manufacturer': self.get_value_data(
                                address.get('address').get('id_manufacturer')) == '1' and address.get('address').get(
                                'id_manufacturer') or False,
                            ...
                            if state_id > 0:
                                ...
                                if not state_ids:
                                    ...
                                    state_id = state_obj.create(
                                        {'name': state_name, 'code': state_code, 'country_id': country_ids[0].id})
                                else:
                        ...
                        if not address_ids:
                            parent_ids = res_partner_obj.search(
                                [('presta_id', '=', self.get_value_data(address.get('address').get('id_customer')))])
                            if parent_ids:
                                try:
    ...
    def create_presta_product(self, product, prestashop):
        ...
        log_id1 = False
        category_obj = self.env['prestashop.category']
    ...
    def import_products_images(self):
        ...
        for shop in self:
            ...
            if last_import_date:
                ...
                products = prestashop.get('products', options={'filter[date_upd]': last_imported_date, 'date': '1',
                                                               'filter[id_shop_default]': shop.presta_id})
            ...
            for prd_ids in prod_list:
                ...
                if prod_data:
                    ...
                    for prod_pos in prod_data.findall('./product'):
                        ...
                        image_lst = [image.get('{http://www.w3.org/1999/xlink}href') for image in
                                     prod_pos.findall('./associations/images/image')]
                        image_ids_list = []
                        ...
                        for image_ids in image_lst:
                            ...
                            if file_contain and tmpl_ids:
                                image = base64.b64encode(file_contain)
                                image_prd_ids = product_img_obj.search(
                                    [('product_imgs', '=', tmpl_ids[0].id), ('image_url', '=', image_ids)])
        ...
        for shop in self:
            ...
            if last_import_inventory:
                ...
                products = prestashop.get('products',
                                          options={'filter[date_upd]': last_imported_inventory, 'date': '1'}, )
            else:
                products = prestashop.get('products')
                    ...
                    for stock in stocks:
                        ...
                        if stock_id:
                            ...
                            if self.get_value_data(
                                    stock_detail.get('stock_available').get('id_product')) not in prod_ids:
                                continue
                            shop.createInventory(stock_detail.get('stock_available'), shop.warehouse_id.lot_stock_id.id,
                                                 prestashop)
        ...
        for shop in self:
            ...
            order_data = prestashop.get('orders', options={
                'filter[date_upd]': "[{},{}]".format(
                    self.last_prestashop_order_import_date,
                    str(datetime.now())
                ),
                'date': '1',
                'sort': '[id_DESC]'
            ...
            for shop in self:
                ...
                if message.get('customer_threads') and message.get('customer_threads').get('customer_thread'):
                    ...
                    for msg_id in messages:
                        thread_data = prestashop.get('customer_threads',
                                                     self.get_value_data(msg_id.get('attrs').get('id')))
                        order_msg_vals = {
                        ...
                        if self.get_value_data(thread_data.get('customer_thread').get('id_customer')):
                            p_ids = res_obj.search([('presta_id', '=', self.get_value_data(
                                thread_data.get('customer_thread').get('id_customer')))])
                            if p_ids:
                                order_msg_vals.update({'customer_id': p_ids[0].id, })
                            else:
                                if self.get_value_data(thread_data.get('customer_thread').get('id_customer')) != '0':
                                    customer_detail = prestashop.get('customers', self.get_value_data(
                                        thread_data.get('customer_thread').get('id_customer')))
                        ...
                        if order_presta_id:
                            order = sale_obj.search([('presta_id', '=', self.get_value_data(
                                thread_data.get('customer_thread').get('id_order')))])
                            if order:
                                ...
                                if self.get_value_data(thread_data.get('customer_thread').get('id_order')) != '0':
                                    order_detail = prestashop.get('orders', self.get_value_data(
                                        thread_data.get('customer_thread').get('id_order')))
                                    order_data_ids = order_detail.get('order')
                        ...
                        for each in carts:
                            ...
                            if cart_id:
                                ...
                                cart_vals = {
                                    ...
                                    'partial_use': bool(
                                        int(self.get_value_data(cart_data.get('cart_rule').get('partial_use')))),
                                    'minimum_amount': self.get_value_data(
                                        cart_data.get('cart_rule').get('minimum_amount')),
                                    'free_shipping': bool(
                                        int(self.get_value_data(cart_data.get('cart_rule').get('free_shipping')))),
                                    # 'name' : cart_data.get('cart_rule').get('name').get('language').get('value'),
                                    'name': self.get_value_data(
                                        self.get_value(cart_data.get('cart_rule').get('name').get('language'))),
                                    'presta_id': cart_id,
                                }
                                carts_ids = cart_obj.search(
                                    [('code', '=', self.get_value_data(cart_data.get('cart_rule').get('code')))])
                                if not carts_ids:
                                ...
                                if not data:
                                    self.env.cr.execute(
                                        "insert into cart_shop_rel values(%s,%s)" % (carts_id.id, shop.id))
            except Exception as e:
                if self.env.context.get('log_id'):
                    log_id = self.env.context.get('log_id')
                    self.env['log.error'].create({'log_description': str(e), 'log_id': log_id})
        ...
        for shop in self:
            ...
            try:
                ...
                if last_import_catalog:
                    ...
                    catalog_rule = prestashop.get('specific_price_rules',
                                                  options={'filter[date_upd]': last_imported_catalog, 'date': '1'})
                    ...
                    if catalog_rule.get('specific_price_rules') and catalog_rule.get('specific_price_rules').get(
                        ...
                        for each in catalog_rules:
                            ...
                            if catalog_id:
                                ...
                                from_date = False
                                if not self.get_value_data(
                                        catalog_data.get('specific_price_rule').get('from')) == '0000-00-00 00:00:00':
                                    from_date = self.get_value_data(catalog_data.get('specific_price_rule').get('from'))
                                to_date = False
                                if not self.get_value_data(
                                        catalog_data.get('specific_price_rule').get('to')) == '0000-00-00 00:00:00':
                                ...
                                rule_vals = {
                                    ...
                                    'from_quantity': self.get_value_data(
                                        catalog_data.get('specific_price_rule').get('from_quantity')),
                                    'price': self.get_value_data(catalog_data.get('specific_price_rule').get('price')),
                                    'reduction': self.get_value_data(
                                        catalog_data.get('specific_price_rule').get('reduction')),
                                    'reduction_type': self.get_value_data(
                                        catalog_data.get('specific_price_rule').get('reduction_type')),
                                    'from_date': from_date,
                                    'to_date': to_date,
                                ...
                                rule_ids = catalog_price_obj.search([('name', '=', self.get_value_data(
                                    catalog_data.get('specific_price_rule').get('name')))])
                                if not rule_ids:
                                ...
                                if not data:
                                    self.env.cr.execute(
                                        "insert into catalog_shop_rel values(%s,%s)" % (rule_id.id, shop.id))
            except Exception as e:
                if self.env.context.get('log_id'):
                    log_id = self.env.context.get('log_id')
                    self.env['log.error'].create({'log_description': str(e), 'log_id': log_id})
                    ...
                    log_id_obj = self.env['prestashop.log'].create(
                        {'all_operations': 'import_catalog_rules',
            ...
            try:
                ...
                if shop.prestashop_last_update_category_date:
                    categ_ids = categ_obj.search(
                        [('write_date', '>=', shop.prestashop_last_update_category_date), ('id', 'in', fetch_categ)])
                else:
    ...
    def update_order_status(self):
        ...
        for shop in self:
            try:
                prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,
                                                      shop.prestashop_instance_id.webservice_key or None)
                query = "select * from sale_order o, presta_order_status ps  where o.presta_id is not null and ps.name in ('Awaiting check payment','Awaiting bank wire payment','Awaiting Cash On Delivery validation','Processing in progress') and shop_id = %s" % (
                    shop.id)
                self.env.cr.execute(query)
                ...
                if fetch_orders is not None:
                    ...
                    if shop.prestashop_last_update_order_status_date:
                        sale_order_ids = sale_order.search([('id', 'in', fetch_orders), ('order_status.name', 'in',
                    ...
                    for status in os_status:
                        ...
                        if state_ids:
                            ...
                            st_id = status_obj.create(
                                {'name': self.get_value(
                                    orders_status_lst.get('order_state').get('name').get('language')).get('value'),
                    ...
                    for sale_order in sale_order_ids:
                        ...
                        order.get('order').update({
                            ...
                            'id_carrier': sale_order.carrier_prestashop and str(
                                sale_order.carrier_prestashop.presta_id),
                        ...
                        if sale_order.invoice_status == 'invoiced':
                            order.get('order').get('total_paid_tax_incl').update(
                                {'value': str(sale_order.amount_total)})
                            order.get('order').get('total_paid_tax_excl').update(
                                {'value': str(sale_order.amount_untaxed)})
                        ...
                        if sale_order.order_status.name in ['Awaiting check payment', 'Awaiting bank wire payment',
                                                            'Awaiting Cash On Delivery validation',
                                                            'Processing in progress']:
                ...
                if self.env.context.get('log_id'):
                    ...
                    log_id_obj = self.env['prestashop.log'].create(
                        {'all_operations': 'update_order_status',
                         'error_lines': [(0, 0, {'log_description': str(e), })]})
                    log_id = log_id_obj.id
                new_context = dict(self.env.context)
                new_context.update({'log_id': log_id})
