...
var CancelOrderPopup = PopupWidget.extend({
    ...
    show: function(options){
        ...
        options = options || {};
        this._super(options);
        this.order_tobe_cancel = options.order_tobe_cancel;
        this.pack_lots_by_line = options.pack_lots_by_line;
        this.pack_lots = options.pack_lots
        this.lines = options.lines
        this.line = options.line
        this.quantity_by_lot = options.quantity_by_lot;
        this.product_by_quantity = options.product_by_quantity;
        var line;
        self.update_summary();
        self.renderElement();
    },
    get_lots_by_line : function(line){
        var self = this;
        return this.pack_lots_by_line[line.id]
    },
    ...
    click_confirm: function(){
        ...
        _.each($('.ac_selected_product:checked'), function(item){
            ...
            var lot_name = $(item).data('lot_name');
            var line = new models.Orderline({}, {pos: self.pos, order: selectedOrder, product: product});
            line.set_cancel_process(orderline.order_id);
            line.set_cancel_item(true);
            line.set_cancel_item_id(orderline.id);
            if((orderline.qty - qty) <= 0){
                line.set_line_status("full");
            } else {
                line.set_line_status("partial");
            }
            if(product.type != "service"){
                line.set_consider_qty(orderline.qty - qty);
            }
            if(product.tracking != 'none'){
                line.set_quantity(qty);
                for(var i=0; i < qty; i++){
                    line.pack_lot_lines.add(new models.Packlotline({}, {'order_line': line}));
                    line.pack_lot_lines.models[i].attributes.lot_name = lot_name
                }
            }else{
                line.set_quantity(-1 * qty);
            }
        ...
        if(temp_orderline_ids.length > 0){
            ...
            selectedOrder.set_remaining_amount(self.order_tobe_cancel.amount_due)
            ...
            selectedOrder.set_amount_paid((new_order_total - (this.order_tobe_cancel.amount_total - this.order_tobe_cancel.amount_due)));
            ...
            if(self.refundable_total > 0){
                ...
                selectedOrder.set_amount_tax(0)
            }
            for(var i=0 ; i < selectedOrder.orderlines.length ; i++){
                if(selectedOrder.orderlines.models[i].has_product_lot){
                    var old_quantity = selectedOrder.orderlines.models[i].get_quantity()
                    selectedOrder.orderlines.models[i].set_quantity(Number(old_quantity) * -1)
                }
            }
    ...
    get_product_image_url: function(product){
        return window.location.origin + '/web/image?model=product.product&field=image_128&id='+product;
    },
    ...
    update_summary: function(){
        ...
        this.el.querySelector('.cancel_order_summary .new_amount_due > .value').textContent = this.format_currency((new_order_total - (self.order_tobe_cancel.amount_total - self.order_tobe_cancel.amount_due)) > 0 ? (new_order_total - (self.order_tobe_cancel.amount_total - self.order_tobe_cancel.amount_due)) : 0.00);
